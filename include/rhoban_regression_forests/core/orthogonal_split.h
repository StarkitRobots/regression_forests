...
public:
  ...
  void splitEntries(const Eigen::MatrixXd& input, Eigen::MatrixXd* lower_output, Eigen::MatrixXd* upper_output);

  /// Generate the lower and upper space
  void splitSpace(const Eigen::MatrixXd& space, Eigen::MatrixXd* lower_space, Eigen::MatrixXd* upper_space);
};
}  // namespace regression_forests
