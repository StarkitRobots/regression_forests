...
namespace Math {
  namespace RegressionTree {
    ...
    protected:
      void fillProjection(std::vector<std::vector<Eigen::VectorXd>>& out,
                          RegressionNode * currentNode,
                          const std::vector<int>& freeDimensions,
                          Eigen::MatrixXd& limits);

      void addSubTree(RegressionNode * node,
                      Eigen::MatrixXd& limits,
    ...
    public:
      ...
      getMaxPair(const Eigen::MatrixXd& limits) const;
      ...
      static std::unique_ptr<RegressionTree>
      avgTrees(const RegressionTree& t1,
