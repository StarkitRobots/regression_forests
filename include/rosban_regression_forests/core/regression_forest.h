...
namespace Math {
  namespace RegressionTree {
    ...
    public:
      ...
      // maxLeafs is used to avoid growing an oversized tree. It is not activated by default (0 value)
      // When preFilter is activated, each tree is projected before being merged
      // in the final result
      std::unique_ptr<RegressionTree> unifiedProjectedTree(const Eigen::MatrixXd& limits,
                                                           size_t maxLeafs = 0,
                                                           bool preFilter = false,
                                                           bool parallelMerge = false);
