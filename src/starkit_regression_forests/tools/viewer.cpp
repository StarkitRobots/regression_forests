...
#include <starkit_utils/string_tools.h>
...
{
Viewer::Viewer(const std::string& forest_path, const std::string& config_path, unsigned int width, unsigned int height)
  : starkit_viewer::Viewer(width, height), forest(new Forest()), dim_index(-1), sub_dim_index(-1)
{
  ...
  dim_names = starkit_utils::split_string(lines[0], ',');
  mins = starkit_utils::split_string(lines[1], ',');
  maxs = starkit_utils::split_string(lines[2], ',');
...
void Viewer::increaseValue(double ratio)
{
  ...
  if (current_limits(dim_index, 0) < min)
    current_limits(dim_index, 0) = min;
  if (current_limits(dim_index, 1) > max)
  ...
  if (current_limits(dim_index, 0) != old_min || current_limits(dim_index, 1) != old_max)
  {
    updateCorners();
  }
}
...
void Viewer::updateCorners()
{
  ...
  if (freeDims.size() == 0)
  {  // Special mode, print value of each tree
  ...
  std::cout << "\tMin: " << projectionMin.first << " at " << projectionMin.second.transpose() << std::endl;
  std::cout << "\tMax: " << projectionMax.first << " at " << projectionMax.second.transpose() << std::endl;
