...
#include "starkit_regression_forests/core/pruning.h"
...
double Forest::getValue(const Eigen::VectorXd& input, AggregationMethod method) const
{
  double sum = 0;
...
std::vector<double> Forest::getValues(const Eigen::VectorXd& input, AggregationMethod method) const
{
...
double Forest::getVar(const Eigen::VectorXd& input, AggregationMethod method) const
{
  double sum = 0;
  std::vector<double> values = getValues(input, method);
  for (double val : values)
...
double Forest::getRandomizedValue(const Eigen::VectorXd& input, std::default_random_engine& engine) const
{
...
{
  int bytes_written = 0;
  bytes_written += starkit_utils::write<int>(out, trees.size());
...
int Forest::read(std::istream& in)
{
  ...
  int bytes_read = 0;
  int nb_trees;
  bytes_read += starkit_utils::read<int>(in, &nb_trees);
...
{
  if (str == "All")
    return Forest::AggregationMethod::All;
  if (str == "AutoCut")
    return Forest::AggregationMethod::AutoCut;
...
std::string aggregationMethod2Str(Forest::AggregationMethod method)
{
  switch (method)
  {
    case Forest::AggregationMethod::All:
      return "All";
    case Forest::AggregationMethod::AutoCut:
      return "AutoCut";
    default:
    {
      std::ostringstream oss;
