#include "starkit_regression_forests/core/node.h"
...
int Node::write(std::ostream& out) const
{
  ...
  if (isLeaf())
  {
    bytes_written += starkit_utils::write<char>(out, 1);
    bytes_written += a->write(out);
  }
  else
  {
    bytes_written += starkit_utils::write<char>(out, 0);
    bytes_written += starkit_utils::write<int>(out, s.dim);
    bytes_written += starkit_utils::write<double>(out, s.val);
...
int Node::read(std::istream& in)
{
  ...
  int bytes_read = 0;
  char is_leaf;
  bytes_read += starkit_utils::read<char>(in, &is_leaf);
  if (is_leaf == 1)
  {
  ...
  {
    // Read split
    bytes_read += starkit_utils::read<int>(in, &s.dim);
    bytes_read += starkit_utils::read<double>(in, &s.val);
    // Read childs
    lowerChild = new Node();
