...
{
  int bytes_written = 0;
  bytes_written += starkit_utils::write<int>(out, factors.rows());
  bytes_written += starkit_utils::writeArray<double>(out, factors.rows(), factors.data());
...
int PWLApproximation::read(std::istream& in)
{
  int bytes_read = 0;
  int nb_factors;
  bytes_read += starkit_utils::read<int>(in, &nb_factors);
  factors = Eigen::VectorXd(nb_factors);
  bytes_read += starkit_utils::readArray<double>(in, nb_factors, factors.data());
  return bytes_read;
}
