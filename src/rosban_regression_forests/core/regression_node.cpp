#include "RegressionNode.hpp"

#include "CompositeApproximation.hpp"

namespace Math {
  namespace RegressionTree {

    RegressionNode::RegressionNode()
      : a(NULL), father(NULL), upperChild(NULL), lowerChild(NULL)
    {
    }

    RegressionNode::RegressionNode(RegressionNode * father_)
      : a(NULL), father(father_), upperChild(NULL), lowerChild(NULL)
    {
    }
    ...
    {
      if (isLeaf()) return 0;
      int childMaxDim = std::max(lowerChild->maxSplitDim(),
                                 upperChild->maxSplitDim());
    ...
    RegressionNode::getMaxPair(Eigen::MatrixXd& limits) const
    {
      std::pair<double, Eigen::VectorXd> best;
      best.first = std::numeric_limits<double>::lowest();
    ...
    void RegressionNode::updateMaxPair(Eigen::MatrixXd& limits,
    ...
    RegressionNode::project(const std::vector<int>& freeDimensions,
                            const Eigen::MatrixXd& limits)
    {
      if (a == NULL) {
        throw std::runtime_error("RegressionNode::project: no approximation available on the current node");
      }
    ...
    void RegressionNode::parallelMerge(RegressionNode& node,
                                       ...
                                       Eigen::MatrixXd& limits)
    {
      ...
      if (t1.isLeaf()) {
        ...
        if (t2.isLeaf()) {
          node.a = CompositeApproximation::weightedMerge(t1.a->clone(), w1,
                                                         t2.a->clone(), w2);
        }
      ...
      // 2. T1 is a split node
        ...
        // 2.1 limits forces to choose the lower child (max is higher or equal to splitVal)
        if (max <= sVal) {
          parallelMerge(node, t2, *t1.lowerChild, w2, w1, limits);
        }
        // 2.2 limits forces to choose the upper child (min is strictly lower than splitVal)
        else if (min > sVal) {
