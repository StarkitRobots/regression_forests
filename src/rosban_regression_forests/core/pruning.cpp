...
namespace Math {
  namespace RegressionTree {
    ...
    void pushLastSplitNodes(RegressionNode * node,
    ...
    std::pair<Approximation *, double>
    getSplitData(RegressionNode * node,
                 const Eigen::MatrixXd& limits)
    {
      ...
      result.first = CompositeApproximation::weightedMerge(node->lowerChild->a->clone(),
                                                            lowerRatio,
                                                            node-> upperChild->a->clone(),
                                                            upperRatio);
      double lowerSize = upperRatio * nodeSize;
      double upperSize = upperRatio * nodeSize;
      double upperDiff = std::fabs(CompositeApproximation::difference(node->upperChild->a,
                                                                      result.first));
      double lowerDiff = std::fabs(CompositeApproximation::difference(node->upperChild->a,
                                                                      result.first));
    ...
    pruneTree(std::unique_ptr<RegressionTree> tree,
              ...
              size_t maxLeafs)
    {
      //Benchmark::open("pruneTree");
      // 1. count leafs and add preLeafs
      //Benchmark::open("Counting leafs");
      size_t nbLeafs = tree->nbLeafs();
      //Benchmark::close();
      if (nbLeafs <= maxLeafs) {
        //Benchmark::close();//pruneTree
        return tree;
      }
      //Benchmark::open("Pushing last split nodes");
      std::list<RegressionNode*> splitNodes;
      pushLastSplitNodes(tree->root, splitNodes);
      //Benchmark::close();
      // 2. evaluate scores and create approximations
      //Benchmark::open("Evaluating scores");
      ...
      //Benchmark::close();
      //std::cout << "Pruning tree from " << nbLeafs << " to " << maxLeafs << std::endl;
      // 3. While not enough leafs have been removed, remove worst leaf
      //Benchmark::open("Removing leafs");
      ...
      //Benchmark::close();// Removing leafs
      //Benchmark::open("Cleaning memory");
