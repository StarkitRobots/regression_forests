...
namespace Math {
  namespace RegressionTree {
    ...
    PWLApproximation::PWLApproximation(const Eigen::VectorXd& factors_)
      : factors(factors_)
    {
    }
                                       ...
                                       const std::vector<double>& outputs)
    {
      ...
      //factors = a.fullPivHouseholderQr().solve(b);// Weird results when testing this one
    ...
    double PWLApproximation::eval(const Eigen::VectorXd& state) const
    {
      if (state.rows() != factors.rows() - 1) {
        ...
        oss << "PWLApproximation::eval -> received a state of dim : "
            << state.rows() << " expecting state of dim : "
            << (factors.rows() - 1);
